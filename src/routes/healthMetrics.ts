...
healthMetricsRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const allowedMetricTypes = new Set(['steps', 'water']);
...
healthMetricsRouter.post(
  '/current-user/import/apple-health/water',
  requireCognitoAuth,
  async (req, res) => {
    const sub = req.cognito?.sub;
    ...
    try {
      ...
      if (metricType !== 'water') {
        return res.status(400).json({
          message: 'metricType must be "water"'
        });
      }
      ...
      if (existing) {
        ...
        await userIntegrations.updateOne(
          ...
          },
          {
            $set: {
              updatedAt: now,
              'lastSync.waterImportedAt': now,
              'lastSync.waterDate': date
            }
          }
        );
        ...
        const existingLatestWaterDate =
          typeof existingProfile?.latestWaterDate === 'string'
            ? existingProfile.latestWaterDate
            : null;

        const shouldUpdateProfileWater =
          !existingLatestWaterDate || date >= existingLatestWaterDate;

        if (shouldUpdateProfileWater) {
          await userProfiles.updateOne(
            { userId: actor._id },
            {
              $set: {
                latestWaterMl: roundedMilliliters,
                latestWaterDate: date,
                updatedAt: now
              }
            }
          );
      ...
      console.error('[healthMetrics/importAppleHealthWater] failed', {
        sub: sub ?? null,
        error: err instanceof Error ? err.message : String(err),
        stack: err instanceof Error ? err.stack : undefined
      });
      ...
      return res.status(500).json({
        message: 'Failed to import Apple Health water'
      });
    }
  }
);
